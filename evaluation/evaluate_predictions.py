...
def evaluate_predictions(input_path, output_dir=None):
    ...
    # decision with the pre-judge checkpoint so a later tribunal change is
    # never misreported as a debate correction or harm.
    debate_final_column = (
        "pre_judge_prediction"
        if "pre_judge_prediction" in df.columns
        else "final_prediction"
    )
    if {"initial_prediction", debate_final_column}.issubset(df.columns):
        debated = df[
            debate_mask
            & df["initial_prediction"].isin(LABELS)
            & df[debate_final_column].isin(LABELS)
        ].copy()
    else:
        debated = pd.DataFrame()
    if not debated.empty:
        debated["initial_correct"] = debated["initial_prediction"] == debated["ground_truth"]
        debated["post_debate_prediction"] = debated[debate_final_column]
        debated["final_correct"] = (
            debated["post_debate_prediction"] == debated["ground_truth"]
        )
    ...
    tribunal_requested_mask = (
        judge_requested_mask
        & df.get("judge_mode", pd.Series("", index=df.index)).eq("tribunal")
    )
    ...
    evidence_columns = (
        ...
        "debate_visual_witness_count", "debate_visual_witness_ids",
